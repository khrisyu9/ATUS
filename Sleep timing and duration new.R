...
for (s.year in c(2015)){# 2003:2018) {
  file.name = sprintf("D:/galit/18th Debug/actlog_galit.debug.%s.csv",s.year)
  ...
  for (id in gid2) {
    ...
    data0 = data %>%
      group_by(tucaseid) %>%
      filter(tuactivity_n >= max(primary.sleep.number), keep == 1) %>%
      filter(start < min(stop)+2*60*60, trcode == 10101)
  ...
  gid3 = unique((g.actlog3 %>% filter(tucaseid %in% gid,
                                      primary.sleep == "primary sleep",
                                      last == 0))$tucaseid)
  g.actlog4 = g.actlog3 %>% filter(!(tucaseid %in% gid3)) %>% ungroup()
  ...
  g.primary.id = unique((g.actlog4%>%filter(primary.sleep == "primary sleep"))$tucaseid)

  primary.start = g.actlog4 %>%
    group_by(tucaseid) %>%
    mutate(primary.sleep.number = ifelse(primary.sleep == "primary sleep", tuactivity_n, 0),
           primary.sleep.number = ifelse(primary.sleep.number == max(primary.sleep.number),
                                         primary.sleep.number, 0)) %>%
  ...
  g.actlog0 = g.actlog4 %>%
    ...
    ungroup() 
  a = g.actlog0 %>% filter(tucaseid %in% gid3)
