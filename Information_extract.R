...
###############################################################################
############## Collect the activities 1 hour before primary sleep #############
###############################################################################
  ...
  foreach(
    s.year = 2003:2018, .combine = "rbind") %dopar% {
      ...
      actlog_new = actlog_new %>% filter(primary.sleep == "other sleep") %>% 
        ...
        # get activities one hour before bed
        filter(stop > sleep_timing - 60*60) %>%
        mutate(duration = ifelse(start < sleep_timing - 60*60, 
                                 as.numeric(stop - sleep_timing + 60*60, units = "mins"),
...
write.csv(act_before_sleep1, "D:/galit/18th Debug/act_duration.1hr.debug.csv")
